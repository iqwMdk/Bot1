...
def get_user(user_id):
    ...
    res = cursor.fetchone()
    if not res:
        cursor.execute("INSERT INTO users (user_id, wallet, bank) VALUES (?, 100, 0)", (user_id,))
        conn.commit()
        return 100, 0
    return res[0], res[1]
...
async def help_cmd(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!عقاراتي` - لعرض العقارات التي تمتلكها أرباحها\n"
...
async def رصيدي(ctx):
    ...
    embed.add_field(name="💵 الكاش (المحفظة)", value=f"{wallet:,} ريال", inline=True)
    embed.add_field(name="🏦 البنك", value=f"{bank:,} ريال", inline=True)
    embed.add_field(name="📊 المجموع", value=f"{(wallet + bank):,} ريال", inline=False)
    await ctx.send(embed=embed)
...
async def daily_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        await ctx.send(f"⏳ يمكنك أخذ اليومية بعد: **{hours} ساعة و {minutes} دقيقة**")
...
async def عمل(ctx):
    ...
    jobs = ["عملت في المقهى", "قمت بتوصيل بعض الطلبات", "صلحت جهاز كمبيوتر", "عملت في كتابة المقالات"]
...
async def work_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        await ctx.send(f"⏳ أنت متعب الآن! يمكنك العمل مجدداً بعد **{minutes} دقيقة**.")
...
async def العقارات(ctx):
    cursor.execute("SELECT property_id, name, price, daily_income, owner_id, is_for_sale, sale_price FROM properties")
    ...
    embed = discord.Embed(title="🏢 سوق العقارات والمنشآت", color=discord.Color.blue())
    for prop in props:
        ...
        if owner_id:
            ...
            owner_name = owner_user.display_name if owner_user else f"مستخدم ({owner_id})"
            if is_sale:
...
async def عقاراتي(ctx):
    cursor.execute("SELECT property_id, name, daily_income, is_for_sale, sale_price FROM properties WHERE owner_id = ?", (ctx.author.id,))
    props = cursor.fetchall()
    if not props:
        await ctx.send("🏠 أنت لا تمتلك أي عقارات حالياً!")
        return

    embed = discord.Embed(title=f"🏢 عقارات {ctx.author.display_name}", color=discord.Color.purple())
    for prop in props:
        p_id, name, income, is_sale, sale_price = prop
        sale_status = f"متاح للبيع بسعر {sale_price:,} ريال" if is_sale else "غير معروض للبيع"
        embed.add_field(
            name=f"#{p_id} - {name}",
...
async def شراء_عقار(ctx, property_id: int = None):
    if not property_id:
        await ctx.send("❌ يرجى كتابة رقم العقار المراد شراؤه. مثال: `!شراء_عقار 1`")
        return

    cursor.execute("SELECT property_id, name, price, owner_id, is_for_sale, sale_price FROM properties WHERE property_id = ?", (property_id,))
    ...
    if owner_id is None:
        if wallet < price:
            await ctx.send(f"❌ لا تملك المبلغ الكافي لشراء {name}. السعر المطلوب: **{price:,} ريال**")
            return
        update_wallet(ctx.author.id, -price)
        cursor.execute("UPDATE properties SET owner_id = ? WHERE property_id = ?", (ctx.author.id, p_id))
        conn.commit()
        await ctx.send(f"🎉 مبروك! قمت بشراء **{name}** بمبلغ **{price:,} ريال** وحصرياً أصبحت مالكه!")
        return
    ...
    if is_sale:
        ...
        if wallet < sale_price:
            await ctx.send(f"❌ لا تملك المبلغ الكافي لشراء العقار من المزاد. المطلوب: **{sale_price:,} ريال**")
            return

        # نقل الأموال والملكية
        update_wallet(ctx.author.id, -sale_price)
        update_wallet(owner_id, sale_price) # إعطاء المالك السابق فلوسه
        cursor.execute("UPDATE properties SET owner_id = ?, is_for_sale = 0, sale_price = 0 WHERE property_id = ?", (ctx.author.id, p_id))
        conn.commit()
        await ctx.send(f"🤝 تم الشراء! انتقلت ملكية **{name}** إليك من المالك السابق بمبلغ **{sale_price:,} ريال**.")
        return
...
async def ارباح_العقارات(ctx):
    cursor.execute("SELECT daily_income FROM properties WHERE owner_id = ?", (ctx.author.id,))
    ...
    await ctx.send(f"📈 تم جمع أرباح عقاراتك اليومية بنجاح! تم إضافة **{total_income:,} ريال** لملاحظتك/الكاش.")
...
async def claims_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        hours = int(error.retry_after // 3600)
        minutes = int((error.retry_after % 3600) // 60)
        await ctx.send(f"⏳ لقد جمعت أرباحك مؤخراً! يمكنك الجمع مجدداً بعد: **{hours} ساعة و {minutes} دقيقة**")

@bot.command()
async def عرض_للبيع(ctx, property_id: int = None, price: int = None):
    if not property_id or not price or price <= 0:
        await ctx.send("❌ الصيغة الصحيحة: `!عرض_للبيع [رقم_العقار] [السعر_المطلوب]`")
        return

    cursor.execute("SELECT name, owner_id FROM properties WHERE property_id = ?", (property_id,))
    prop = cursor.fetchone()
    if not prop or prop[1] != ctx.author.id:
        await ctx.send("❌ أنت لا تمتلك هذا العقار لكي تعرضه للبيع!")
        return

    cursor.execute("UPDATE properties SET is_for_sale = 1, sale_price = ? WHERE property_id = ?", (price, property_id))
    conn.commit()
    await ctx.send(f"🏷️ تم عرض عقارك **({prop[0]})** في السوق/المزاد بسعر **{price:,} ريال**.")
...
async def الغاء_البيع(ctx, property_id: int = None):
    ...
    cursor.execute("UPDATE properties SET is_for_sale = 0, sale_price = 0 WHERE property_id = ?", (property_id,))
    conn.commit()
...
async def اضافة_عقار(ctx, name: str, price: int, daily_income: int):
    """مثال الاستخدام: !اضافة_عقار "فندق_الفخامة" 50000 1500"""
    cursor.execute("INSERT INTO properties (name, price, daily_income) VALUES (?, ?, ?)", (name, price, daily_income))
    conn.commit()
    await ctx.send(f"✅ تم إضافة عقار جديد: **{name}** بسعر **{price:,} ريال** وربح يومي **{daily_income:,} ريال**.")
